<template>
  <div class="steps" v-if="steps">
    ...
    <md-dialog :md-active.sync="editTogal">
      <md-dialog-title>Update Step</md-dialog-title>
      <md-dialog-content>
    ...
    <md-dialog :md-active.sync="addTogal">
      <md-dialog-title>Add Step</md-dialog-title>
      <md-dialog-content>
    ...
    <md-table md-sort="name" md-sort-order="asc" md-card>
      ...
      >
        <md-button class="md-primary md-raised" @click="add()">Create new step</md-button>
      </md-table-empty-state>
      <md-table-row>
      ...
      <md-table-row v-for="step in steps.data" :key="step.id">
        ...
        <md-table-cell md-label="Actions">
          ...
          <md-button class="md-icon-button md-accent" @click="remove(step.id)">
            <md-icon>delete</md-icon>
          </md-button>
        </md-table-cell>
      </md-table-row>
    </md-table>
    <pagination :limit="4" :data="steps" @pagination-change-page="get"></pagination>
    <md-button class="md-fab md-primary add-btn" @click="add()">
      <md-icon>add</md-icon>
      <md-tooltip>Add new step</md-tooltip>
    </md-button>
  </div>
</template>
  ...
  methods: {
    ...
    get(page = 1) {
      axios
        .get("steps?page=" + page)
        .then((res) => {
          console.log("step: ", res.data.data);
          this.steps = res.data;
        })
        .catch((err) => {
