...
<div class="shipper-orders">
    <md-card class="no-shadow-bordered">
        ...
        <md-card-content>
            ...
            <table class="table" v-if="dataLoaded">
                ...
                <tbody>
                    <tr v-for="order in orders" :key="order.id">
                        <td>{{order.full_address[0].city}}</td>
                        <td>{{order.full_address[1].city}}</td>
                        <td>{{order.created_at}}</td>
                        <td v-bind:class="{delivered: order.job_with_status.jobstatus.id == 5}">{{order.job_with_status.jobstatus.title}}</td>
            ...
            <md-empty-state v-else v-show="!tempOrder" class="md-primary" md-icon="sentiment_satisfied_alt" md-label="Not available" md-description="Click + icon to get a new quote">
